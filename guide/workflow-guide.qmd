...
The full orchestrator (Pattern 2) is designed for course materials with multi-agent review loops. For research projects, the **simple variant** strips this down to: implement → verify → score → done. No multi-round reviews, no parallel agent spawning. See the "Research Project Variant" section in `.claude/rules/orchestrator-protocol.md`.
...
The knowledge base (`.claude/rules/knowledge-base-template.md`) is the most domain-specific component. For **courses**, it has sections for notation, lecture progression, applications, and design principles. For **research projects**, Section 5 adds templates for estimand registries, mathematical objects, DGP configurations, tolerance thresholds, and output directory structure.
...
| Orchestrator Protocol | `.claude/rules/orchestrator-protocol.md` | Contractor mode: full loop (slides) or simple loop (research) |
| Exploration Folder | `.claude/rules/exploration-folder-protocol.md` | Structured sandbox for experimental work |
| Exploration Fast-Track | `.claude/rules/exploration-fast-track.md` | Lightweight workflow for explorations (60/100 threshold) |
| Session Logging | `.claude/rules/session-logging.md` | Enhanced logging with structured templates + merge-only reporting |
